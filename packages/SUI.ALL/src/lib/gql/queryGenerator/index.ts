...
export function generateCreateText(entity: string, fields: object): string {
  ...
  return `mutation {
  create${capitalize(camelCaseEntity)}(input: {${camelCaseEntity}: {${Object.keys(fields).map(key => `${key}: ${addQuotesIfString((fields as IObjectWithIndex)[key])}`) as unknown as string}}}) {
    ${camelCaseEntity} {
      id
    }
  }
