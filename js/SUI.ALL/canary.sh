...
yarn publish --registry http://verdaccio.smp.sm-soft.ru/ --non-interactive --no-git-tag-version --new-version $VERSION
rm package.json
mv package.json.bkp package.json
printf "yarn add @smsoft/sui-all@$VERSION" | pbcopy
