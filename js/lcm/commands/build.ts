...
async function fatherBuild(packageName: string): Promise<void> {
  ...
  await wrapSpawn("father-build", spawnedProcess);
}

async function tscAlias(packageName: string): Promise<void> {
  const spawnedProcess = child_process.spawn("tsc-alias", ["-p", "tsconfig.json"], {
    cwd: getPackagePath(packageName)
  });

  await wrapSpawn("tsc-alias", spawnedProcess);
}

// Export ... was in found '@sui/all'
// https://github.com/babel/babel/issues/8361
function fixBabelWarning(packageNameOrPath: string) {
