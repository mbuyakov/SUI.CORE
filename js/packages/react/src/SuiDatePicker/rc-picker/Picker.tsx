...
import {changeDateByMask} from "@/SuiDatePicker/suiUtils";
import PickerPanel, {PickerPanelBaseProps, PickerPanelDateProps, PickerPanelTimeProps,} from './PickerPanel';
...
function InnerPicker<DateType>(props: PickerProps<DateType>) {
  ...
  // ============================= Input =============================
  const needToChangeByMask = useFormatAsMask && typeof formatList[0] === 'string';
  const allInputProps = {
    id,
    tabIndex,
    disabled,
    ...
    onChange: ((e): void => triggerTextChange(needToChangeByMask
        ? changeDateByMask(hoverValue || text, e.target.value, formatList[0] as string)
        : e.target.value)
    ),
    autoFocus,
    placeholder,
