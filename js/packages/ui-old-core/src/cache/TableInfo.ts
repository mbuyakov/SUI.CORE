...
export class TableInfo {
  ...
  public async loadColumns(): Promise<void> {
    ...
    await asyncMap(this.colIds, id => ColumnInfoManager.getById(id));
  }
}
...
class _TableInfoManager extends GqlCacheManager<ITableInfo, TableInfo> {
  ...
  protected mapRawItem(item: ITableInfo): ICacheEntry<string, TableInfo> {
    return {
      key: item.id,
      item: new TableInfo(item),
    };
  }
}

export const TableInfoManager: CacheManager<TableInfo, string> = new _TableInfoManager();
